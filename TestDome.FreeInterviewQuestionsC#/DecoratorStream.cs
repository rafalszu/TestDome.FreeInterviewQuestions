...
namespace TestDome.FreeInterviewQuestionsC
{
    ...
    For example, if the DecoratorStream is instantiated with "First line: " as the prefix parameter and Write method is called with UTF-8 byte
    representation of "Hello, world!", it should write "First line: Hello, world!" into the underlying stream.
    */
    public class DecoratorStream : Stream
    {
        ...
        {
            byte[] fullarray;
            byte[] prefixBuffer = new byte[0];
            if(stream.Position == 0) {
                prefixBuffer = System.Text.Encoding.UTF8.GetBytes(prefix);
                fullarray = new byte[prefixBuffer.Length + bytes.Length];
                Buffer.BlockCopy(prefixBuffer, 0, fullarray, 0, prefixBuffer.Length);
            }
            else {
                fullarray = new byte[bytes.Length];
            }

            Buffer.BlockCopy(bytes, 0, fullarray, prefixBuffer.Length, bytes.Length);
            
            stream.Write(fullarray, 0, fullarray.Length);
            stream.Flush();
        }
